...
prompt_pure_preprompt_render() {
	...
	if [ -w $WDIR ]; then
		...
		preprompt_parts+=('%{$bg_bold[red]%}%F{white}%~%f%{$bg_no_bold[default]%}')
	fi
...
#	username='%F{220}%n%f%F{246}@%m%f'

	# show username@host if logged in through SSH
	[[ -n $ssh_connection ]] && username='%F{220}%n%f%F{246}@%m%f' #username='%F{242}%n@%m%f'
